...
public class Days{
    ...
    {
        string path = @"..\..\..\inputs\day12input.txt";
        string[] lines;
        lines = File.ReadAllLines(path);
        List<SpringRow> springRows = new List<SpringRow>();
        foreach(string line in lines){
            springRows.Add(new SpringRow(line));
        }
        long solutions = 0;
        foreach(SpringRow springRow in springRows){solutions += springRow.NumberOfPossibleSolutions();}
        Console.WriteLine("test");
    }
// Day 12 solution taken from https://pastebin.com/djb8RJ85
